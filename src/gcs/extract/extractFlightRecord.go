...
// Inerface
type ExtractFlightRecord struct {
	ex ExtractBucket
}

///////////////////////////////////////////////////
/* ===========================================
GCS BucketからFlightRecordのログを取得
=========================================== */
func (e *ExtractFlightRecord) ExtractFlightRecord() (interface{}, interface{}, error) {
	...
	var pRecords []TABLE.FlightRecordImport
	...
	for i, logs := range logss.Objs {
		for ii, log := range logs {
			...
			var hRecords []TABLE.FlightRecordImport

			//GCSログをパースして、BQへインサート出来るように整形
			fmt.Println("Parse bucket logging now:", logss.Paths[i][ii])
			l := &BUCKET.LogFlightRecord{Log: log, Path: logss.Paths[i][ii]}
			hRecords, parseErrs, err := l.FlightRecordLogParser()
			if err != nil {
				return nil, nil, err
			}
			...
			if len(errIds) != 0 {
				e.ex.ExtractParseErrorLogging(BUCKET.GCS_BUCKET_NAME_FLIGHT_RECORD, logss.Paths[i][ii], errIds) //WARNロギング
			}
